...
$StdoutLogFile = Join-Path (Join-Path $CtiHome 'logs') 'bridge.stdout.log'
$StderrLogFile = Join-Path (Join-Path $CtiHome 'logs') 'bridge.stderr.log'
...
switch ($Command) {
    ...
    'stop' {
        ...
        if ($svc -and $svc.Status -eq 'Running') {
            ...
            $currentPid = Read-Pid
            if (-not $currentPid) { Write-Host "No bridge running"; exit 0 }
            if (Test-PidAlive $currentPid) {
                Stop-Process -Id ([int]$currentPid) -Force
                Write-Host "Bridge stopped"
            } else {
    ...
    'status' {
        ...
        if ($currentPid -and (Test-PidAlive $currentPid)) {
            Write-Host "Bridge process is running (PID: $currentPid)"
