...
function getCodexSandboxMode(): 'read-only' | 'workspace-write' | 'danger-full-access' {
  const mode = process.env.CTI_CODEX_SANDBOX_MODE;
  if (mode === 'read-only' || mode === 'workspace-write' || mode === 'danger-full-access') {
    return mode;
  }
  // Default to writable project sandbox for IM-driven coding sessions.
  return 'workspace-write';
}

function getCodexPathOverride(): string | undefined {
  const value = process.env.CTI_CODEX_EXECUTABLE?.trim();
...
export function buildCodexClientOptionsFromEnv(): Record<string, unknown> {
...
export class CodexProvider implements LLMProvider {
  ...
  private async ensureSDK(): Promise<{ sdk: CodexModule; codex: CodexInstance }> {
    ...
    this.codex = new CodexClass(buildCodexClientOptionsFromEnv());
