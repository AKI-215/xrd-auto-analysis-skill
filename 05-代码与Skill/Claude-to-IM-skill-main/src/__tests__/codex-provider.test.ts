...
describe('CodexProvider', () => {
  ...
  it('defaults sandboxMode to workspace-write', async () => {
  ...
  it('passes CTI_CODEX_SANDBOX_MODE through to the SDK', async () => {
    const old = process.env.CTI_CODEX_SANDBOX_MODE;
  ...
  it('passes codexPathOverride and filters transient CODEX_* env vars', async () => {
    const oldExecutable = process.env.CTI_CODEX_EXECUTABLE;
    const oldThreadId = process.env.CODEX_THREAD_ID;
    process.env.CTI_CODEX_EXECUTABLE = 'C:/custom/codex.exe';
    process.env.CODEX_THREAD_ID = 'transient-thread-id';
    try {
      const { buildCodexClientOptionsFromEnv } = await import('../codex-provider.js');
      const options = buildCodexClientOptionsFromEnv();

      assert.equal(options.codexPathOverride, 'C:/custom/codex.exe');
      assert.ok(options.env, 'Expected env override to be passed');
