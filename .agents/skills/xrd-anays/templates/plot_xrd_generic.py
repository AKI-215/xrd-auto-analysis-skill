...
def plot_with_refs(trace: Trace, refs: list[ReferencePeaks], output: Path, peak_csv: Path | None = None) -> list[tuple[float, float]]:
    ...
    legend_handles = [Line2D([], [], color="#1f1f1f", lw=1.1, label=trace.label)]
    for ref in refs:
        for pos in ref.positions:
            ax.vlines(pos, ref.tick_base, ref.tick_tip, color=ref.color, linewidth=1.0)
        legend_handles.append(Line2D([], [], color=ref.color, marker="|", markersize=14, linestyle="None", markeredgewidth=1.6, label=ref.label))
    ax.set_ylim(-0.14, 1.02)
    ax.set_xlabel(r"2$\theta$ (°)")
    ax.set_ylabel("Normalized intensity")
    ax.legend(handles=legend_handles, frameon=False, loc="upper right", handlelength=0.9, borderaxespad=0.4)
    ax.tick_params(axis="both", which="major", direction="in")
    ax.minorticks_on()
    fig.tight_layout()
    fig.savefig(output, dpi=300, bbox_inches="tight")
...
def parse_refs(values: list[str]) -> list[ReferencePeaks]:
    ...
    for idx, value in enumerate(values):
        ...
        label = name.strip().split("|", 1)[0].strip()
        positions = [float(item) for item in raw_positions.split(",") if item.strip()]
        tick_tip = -0.02 - idx * tick_gap
        tick_base = tick_tip - tick_height
        refs.append(
            ReferencePeaks(
                label=label,
                positions=positions,
